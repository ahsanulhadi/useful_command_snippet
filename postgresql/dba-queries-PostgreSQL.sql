...
-- LIST ALL INDEX and get create index sql
SELECT schemaname, tablename, indexname, indexdef
FROM pg_indexes
WHERE schemaname IN ('dbo', 'acl')
...
SELECT
  (total_exec_time / 1000 / 60) as total_min, -- Total time spent executing the statement, in milliseconds
  max_exec_time/1000 as max_exec_time_sec,
  min_exec_time/1000 as min_exec_time_sec,
  mean_exec_time/1000 as mean_exec_time_sec, -- Mean time spent executing the statement, in milliseconds
  ...
  blk_write_time, -- Total time the statement spent writing data file blocks, in milliseconds (if track_io_timing is enabled, otherwise zero)
  shared_blks_hit, -- Total number of shared block cache hits by the STATEMENT /  the number of blocks already cached when read for the query
  shared_blks_read , -- Total number of shared blocks read by the STATEMENT / the number of blocks that had to be read from the disk to satisfy all calls for this query form.
...
SELECT
	...
	query
FROM pg_stat_activity
WHERE datname = 'eCredentialLive_MaskedProd'
AND NOT (state='idle' OR pid=pg_backend_pid())
;

-- see which query is waiting for whom.
...
-- Find all user and groups
-- gives a view of all users in the system with all inherited role memberships
...
-- Try running a vacuum full on the table and see if that make a difference.
--VACUUM <table-name>;
--REINDEX <table-name>;
--ANALYZE <table-name>;
...
-- Check Index scan and Seq Scan status on Tables *
SELECT
	relname
	, idx_scan
	, seq_scan
...
FROM pg_stat_user_tables
WHERE RELNAME IN ('Table Name') -- Table names
ORDER BY n_live_tup DESC;
...
SELECT
FORMAT('ANALYZE %s.%s;', schemaname, relname) AS cmd
FROM pg_stat_all_tables
WHERE schemaname = '<schema name>'
ORDER BY n_live_tup DESC ;

SELECT
FORMAT('VACUUM (FULL, ANALYZE) %s.%s;', schemaname, relname) AS cmd
FROM pg_stat_all_tables
WHERE schemaname = '<schema name>'
ORDER BY n_live_tup DESC ;
...
https://www.pgconfig.org/#/?max_connections=1000&pg_version=15&environment_name=WEB&total_ram=128&cpus=116&drive_type=SSD&arch=x86-64&os_type=linux
...
Ref: https://www.cybertec-postgresql.com/en/hot-updates-in-postgresql-for-better-performance/
https://stackoverflow.com/questions/71618615/does-postgresql-update-index-even-when-the-updated-columns-arent-the-indexed-on

UPDATE impact on Index
